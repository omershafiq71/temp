                  ...
                  if(Open[1]<Close[1])
                    {
                     ...
                     if(OrderSend(Symbol(),OP_BUY,lotSize,Ask,3,0-(0*Point),0+(0*Point),NULL,magic,0,Red)<0)
                       {
...
void Buy_Order_Latest()
  {
   ...
   for(int i=0;i<OrdersTotal();i++)
     ...
     Print("Total Orders are  ",OrdersTotal());
     Print("latest buy order is at index= ",buy_latest); 
     OrderSelect(buy_latest,SELECT_BY_POS,MODE_TRADES);
     Print("Latest Buy Order's Time is ==========>>>>",OrderOpenTime());
  }
//+------------------------------------------------------------------+
